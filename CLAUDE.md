...
口语问句 → 解析 `metric_name`（`metric_registry.resolve_metric`，空白不敏感）+ 时间窗 + 维度（含 'SKU-xxxx' → 该 SKU；否则全店 `_SHOP_`）→ 查 `mvp_daily_metric` 返**序列 + 一句话简述**（确定性，不调 LLM、不归因）。没听出指标 → 返 29 指标候选清单让老板再说清。
