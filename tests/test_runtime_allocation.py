...
def _repo(tmp_path: Path) -> Path:
    ...
    manifest = {
        "schema_version": 1,
        "canonical_runtime": {"runtime_id": "omni-main", "compose_project": "omni", "database": "omni_vibe_db"},
        "services": {
...
def test_nonoverlapping_allocations_are_isolated_and_conflicts_fail(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    state_dir = tmp_path / "state"
    first = allocation.acquire(
        repo, change_id="change-a", owner="agent-a", path_globs=["services/a/**"], state_dir=state_dir
    )
    second = allocation.acquire(
        repo, change_id="change-b", owner="agent-b", path_globs=["services/b/**"], state_dir=state_dir
    )
    ...
    assert first["environment"]["OMNI_IDENTITY_JWT_SECRET_FILE"] != first["environment"][
...
def test_read_only_allocation_does_not_own_scheduler_or_approval_worker(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
...
def test_expired_record_is_retained_but_does_not_block_reacquire(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    state_dir = tmp_path / "state"
...
def test_hook_path_conflict_read_api_is_read_only_and_reports_stale(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    ...
    same = allocation.resolve_path_conflict(
        repo, "services/a/app.py", "change-a", read_only=False, state_dir=state_dir
    )
...
def test_source_fingerprint_tracks_content_not_mtime_or_secret_files(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
...
def test_lock_file_stays_one_byte_and_release_requires_owner_cas(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    state_dir = tmp_path / "state"
    result = allocation.acquire(
        repo, change_id="change-a", owner="agent-a", path_globs=["services/a/**"], state_dir=state_dir
    ...
    with pytest.raises(allocation.CompareAndSwapConflict):
        allocation.release(repo, allocation_id, owner="agent-a", expected_revision=2, state_dir=state_dir)
