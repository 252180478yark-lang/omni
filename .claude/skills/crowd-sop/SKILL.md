...
**模板查空时别硬出标签（会编出巨量里不存在的词）**，明确降级并告诉老板：

> "KB 里没有 <品类> 的投放标签模板。这种情况我**只出'人群定义 + 圈人思路'，不替你
> 凭空编平台标签**（编了你在巨量后台也勾不到）。Step 4 的策略文里标签那栏会写
> '建议老板在巨量云图/达人广场按此画像自己勾选实际可选项'。可以这样？还是先补份
> 标签模板进 template KB？"
...
> **不出优先级 / 预算分配 / ROI·GMV 预测**（蓝图 §6 硬禁 + CLAUDE.md generate_audience_pack 同禁）：
> crowd-sop 只定"**圈谁 + 用哪些真实标签 + 排除谁**"。"P0/P1/P2 投多少钱、预算比例、预测能带多少 GMV"
> 属投放执行层的事，不是圈人策略层——编了也是没投后数据支撑的拍脑袋，反而误导。老板要投放分配
> 单独走投放复盘/投后数据，不在 crowd-sop 里凑。

**关键约束**：
- 标签词必须是**抖店/巨量后台真实存在**的（如 KB 模板里的，不是 LLM 编的）
- 排除条件必写
- **不写优先级档位、不写预算、不预测 ROI/GMV**（见上方硬禁）

把结果给老板审：

> "策略出来了。核心受众是 X，标签按平台分（巨量/达人广场），排除 Y。看一下哪部分要改。"
...
| 老板说"标签太宽了" | 标签交集选太弱 | 回 Step 3 选更窄的标签交集 |
...
- 出优先级档位 / 预算分配 / 预测 ROI·GMV — 蓝图硬禁（圈人层不碰投放执行/投后预测）
- 不写排除条件 — 圈人没收窄等于白圈
