...
返回 chunks 看 **哪些是直接可用的"卖点框架"**（如"功能-情绪-场景"三段式 / "对比-痛点-解决"等）；如果空命中，**说清楚降级意味着什么**再让老板选：

> "KB 里没找到 <品类> 的卖点模板。我可以用通用框架（功能/情绪/场景）继续，但
> **没品类素材支撑，卖点强度只能靠 SKU 字段硬撑、深度有限**（每条强度评分我会标
> ≤3 + '需补模板'）。两个选择：① 先这样出个粗版你看个方向 ② 你先补一篇模板进
> template KB 我再出扎实版。你要哪个？"
