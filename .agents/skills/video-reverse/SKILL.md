...
> "需要 KE 容器内能访问的文件路径。若文件在宿主机，请按当前运行配置把它放到已挂载目录，或直接上传；不要假设固定桌面目录或固定容器挂载点。"

容器路径转换：以当前 Docker/Host 运行配置中的实际 bind mount 为准；先确认容器可读取路径，再传给工具。
