...
- **要真详情页（手动 B，100% 可靠）**：让老板在自己手机/电脑淘宝里截几张详情页图，放到当前运行配置中 KE 容器可读取的挂载目录，再把实际容器路径传给 `competitor_decompose(local_images=["<容器可读取图片路径>", ...], focus_product="有机酱油")` —— 真实登录态截的图没反爬，5 维全出。老板也可直接把截图贴给我，我直接按 5 维度拆。
...
## 跟 AGENTS.md / 其他 skill 的关系

- 工具实现见 AGENTS.md「竞品调研」节：`competitor_search` / `competitor_decompose`（KE）+ scout-agent `/taobao/*` 端点。
