---
name: script-writer
description: 给一个 SKU 写脚本文本（视频脚本 / 图文脚本 / 直播话术）。老板说"给 SKU-X 写个脚本"、"X 写个直播话术"、"出 X 的图文文案"等，触发标准 5 步走 SOP，串 SKU + 模板 + KB + generate_brief 出脚本。⚠️调味品 SKU 专用、必须基于 omni 真实 SKU/KB 数据：凡只写脚本/文案/直播话术/带货内容一律走本 skill，**不走**通用 copywriting/content-creator/social-content/copy-editing；本 skill 不出图视频，正式纯 AI 种草/软广分别走 ai-planting-video / ai-soft-ad-video，真人制作走 short-video-director-brief，sku-pipeline 只跑前链到 audience_pack_id。
---
...
### Step 2: 找卖点（复用 selling-point-finder 统一矩阵）

**优先看 mvp_sku.owner_selling_points**（老板手填的卖点 JSON 数组）：

```
if SKU.owner_selling_points and len > 0:
    作为 user_initial_points 传给 selling-point-finder / generate_selling_points_matrix
else:
    跑 selling-point-finder：
        get_sku(sku_id)
        generate_selling_points_matrix(sku_id, extra_context="<脚本用途/老板临时要求>")
```

**owner_selling_points 为空时必须先跟老板说清楚再跑**（别静默多花一轮）：

> "002 没填 owner_selling_points。我先跑一遍 selling-point-finder 的统一卖点矩阵
> （会落 matrix_run_id，找完你圈 2-3 条卖点我再往下写脚本）——还是你现在口头给我
> 两三条卖点，我直接用？"

如果老板**之前用过 selling-point-finder skill** 出过这个 SKU 的卖点矩阵，优先复用已采纳的 `matrix_run_id`；没有采纳版但老板明确说"用上次那几条卖点"，可以复用对应 draft，不再重跑。

从矩阵里摘出**三层卖点**（功能/情绪/场景）给老板，让他**圈 2-3 条**进脚本。
...
| 老板说 | 怎么做 |
|---|---|
| "OK / 通过" | 文本脚本完成；如要正式出片，按种草/软广/真人意图交给对应 Skill，不自动续跑 |
...
- 前置：**selling-point-finder** skill（统一卖点矩阵）；如果老板没用过，本 skill Step 2 内置一遍
- 后续：正式纯 AI 种草走 **ai-planting-video**，软广走 **ai-soft-ad-video**，真人团队走 **short-video-director-brief**；只有明确的一次性无血缘试验才直调旧图/视频工具
- **sku-pipeline** 只负责 SKU 前链到 `audience_pack_id`，不承担脚本后的出片
