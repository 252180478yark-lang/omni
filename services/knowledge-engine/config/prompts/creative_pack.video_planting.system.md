...
3. **首屏字幕 ≤ 12 字（最佳 7-10 字）**。**关键澄清**：
   ...
   - 想超 12 字 → 把第一段台词拆成两短句
4. **分镜节奏硬上限**：
   - **单段时长 ≤ 8s**（硬上限，超过即失败 —— 抖音/快手推荐流 8s+ 单段画面停滞完播率断崖）
   - **最佳每段 3-6s**，避免拖
   - **段数下限**：30s 视频 ≥ 5 段 / 45s 视频 ≥ 7 段（除非有真实身份披露段需要 7-8s 慢节奏铺细节，其他都压短）
   - 警告：**严格执行某模块方法论时不能为完整呈现 SOP 而拖长单段**——方法论是裁判标准不是创意发动机（再次确认：宁可压缩说理也不留 10s 停滞画面）
