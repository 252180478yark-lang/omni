...
4. **分镜节奏硬上限**：
   - **单段时长 ≤ 8s**（硬上限，超过即失败 —— 推荐流 8s+ 单段画面停滞完播率断崖）
   - **最佳每段 3-6s**
   - **段数下限**：25-30s 软广 ≥ 5 段（绝大多数模块都该 5-7 段，慢节奏 mini_documentary 例外可 4-6 段但单段仍 ≤ 8s）
   - 警告：**严格执行某模块（如 mini_documentary / pixar_spine）方法论时不能为完整呈现 SOP 而拖长单段**——方法论是裁判标准不是创意发动机
   - `scene_change_max_gap_seconds` 字段必须如实填**最长段秒数**（后端会自己从 scenes time_range 算实际值反验，自报数据自欺会被反作弊抓）
5. 结尾留开放性（禁"完结感"收尾）
