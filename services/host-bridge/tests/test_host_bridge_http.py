...
@pytest.mark.asyncio
async def test_canonical_and_wecom_compatibility_routes_share_local_core(tmp_path, monkeypatch):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        created = await client.post("/api/sessions", json={"project_dir": str(project), "brain_provider": "codex"})
        session_id = created.json()["session_id"]
        opened = await client.post(f"/api/sessions/{session_id}/open")
        prompted = await client.post(f"/api/sessions/{session_id}/prompt", json={"prompt": "hello", "brain_provider": "codex", "request_id": "request:http"})
        visible_denied = await client.post(
            f"/api/v1/host-bridge/sessions/{session_id}/visible-auth",
            json={"provider": "wecom", "url": "https://evil.example/qr", "request_id": "request:evil"},
        )
        visible = await client.post(
            f"/api/v1/host-bridge/sessions/{session_id}/visible-auth",
            json={"provider": "wecom", "url": "https://login.example.test/qr?ticket=secret-value", "request_id": "request:visible"},
        )
        events = await client.get(f"/api/v1/host-bridge/runs/{prompted.json()['run_id']}/events")
        uploaded = await client.post(
            f"/api/v1/host-bridge/sessions/{session_id}/attachments",
            files={"attachment": ("proof.txt", b"attachment proof", "text/plain")},
        )
        downloaded = await client.get(
            f"/api/v1/host-bridge/sessions/{session_id}/attachments/{uploaded.json()['attachment_id']}",
        )
