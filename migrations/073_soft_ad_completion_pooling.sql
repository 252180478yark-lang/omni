-- Migration 073: soft-ad completion winner selection must pool raw counts.
-- A per-asset rate average gives a one-impression asset the same weight as a
-- thousand-impression asset.  Preserve the canonical 34-column view shape and
-- every non-soft-ad behavior while making completion coverage fail closed.
...
evaluated AS (
    SELECT
        ...
        CASE
            WHEN intent = 'soft_ad'
             AND north_star_metric = 'completion_rate'
            THEN eligible_asset_count
            ELSE n_videos
        END AS decision_n_videos,
...
SELECT
    ...
    CASE
        WHEN intent = 'planting' THEN
            CASE WHEN a3_coverage_complete
                 ...
                 ELSE NULL END
        WHEN intent = 'soft_ad'
         AND north_star_metric = 'completion_rate' THEN
    ...
    CASE
        WHEN intent = 'soft_ad'
         AND north_star_metric = 'completion_rate' THEN
            CASE WHEN completion_coverage_complete
                 THEN raw_completion_numerator_sum
                 ELSE NULL END
        ELSE round(legacy_north_star_sum, 4)
    END AS north_star_sum,
    CASE WHEN decision_n_videos < 5
         THEN 'preliminary' ELSE 'sufficient' END AS sample_status,
    production_mode,
    ...
    CASE
        WHEN intent = 'soft_ad'
         AND north_star_metric = 'completion_rate'
        THEN completion_coverage_complete
        ELSE a3_coverage_complete
...
COMMENT ON VIEW pipeline.v_experiment_round_results IS
    'Stable 34-column experiment results. Planting A3 and soft-ad completion north stars pool compatible raw numerators/denominators; other intents retain their previous aggregation.';
