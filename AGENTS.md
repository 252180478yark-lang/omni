...
- 当前产品是老板个人在本机使用的单用户系统。默认运行链路固定为 `local-owner`，不得新增登录页、账号、密码、验证码、会话 cookie、内部 JWT、服务间 HMAC 或内部 Bearer Token；确需调用外部平台/模型时，只允许配置该外部服务本身要求的凭据。
- 本地可信不等于放开网络边界：默认端口必须绑定 loopback，写操作保留 same-origin 校验，删除、发布、付费、外部平台写入、真实成本修改等难恢复动作继续使用显式确认、审计和恢复语义，但不得用账号或密钥充当确认门槛。
