# S7-S14 disposable PostgreSQL verification. Never points at the canonical DB.
$ErrorActionPreference = "Stop"
$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$owner = "codex-s7-s14-verifier"
$changeId = "s7-s14-v" + [DateTimeOffset]::UtcNow.ToString("yyyyMMddHHmmss")
$allocation = $null
...
try {
    ...
    $receiptDir = Join-Path ([System.IO.Path]::GetTempPath()) ("omni-s7-s14-migration-receipts-" + $allocation.allocation.allocation_id)
    ...
    & python -m pytest services/knowledge-engine/tests/test_runtime_trace_postgres.py services/knowledge-engine/tests/test_s7_s14_postgres.py -q
    if ($LASTEXITCODE -ne 0) { throw "real PostgreSQL S7-S14 verification failed" }
    Write-Output "S7_S14_POSTGRES_VERIFIED allocation=$($allocation.allocation.allocation_id) database_mode=disposable"
}
finally {
