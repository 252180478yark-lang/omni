...
function ProviderCard({
  ...
  const handleTest = async () => {
    let testingApiKey = apiKey

    // 输入框为空时，尝试使用系统凭据中已保存的 Key 做连通性测试
    if (!testingApiKey && provider !== 'ollama' && hasStoredApiKey) {
      const result = await ipc.getApiKey?.(provider)
      if (result?.success) {
        testingApiKey = result.data || ''
      }
    }
    ...
    try {
      ...
      if (result?.success && result.data) {
        ...
        setTestResult({ ok: false, error: result?.error || '测试失败' })
      }
    } catch (error) {
